...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n"
                "Usage:  myriadcoin-cli [options] <command> [params]  Send command to " PACKAGE_NAME "\n"
                "or:     myriadcoin-cli [options] -named <command> [name=value]...  Send command to " PACKAGE_NAME " (with named arguments)\n"
                "or:     myriadcoin-cli [options] help                List commands\n"
                "or:     myriadcoin-cli [options] help <command>      Get help for a command\n";
